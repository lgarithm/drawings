...
object *parse_model(const char *str)
{
    ...
    }
    {
        double s;
        point3 p;
        if (sscanf(str, "sphere(%lf, (%lf, %lf, %lf))", &s, &p.x, &p.y, &p.z) ==
            4) {
            if (s > 0)
    ...
    {
        t_vector3 n;
        if (sscanf(str, "plane((%lf, %lf, %lf), (%lf, %lf, %lf))", &n.o.x,
                   &n.o.y, &n.o.z, &n.v.x, &n.v.y, &n.v.z) == 6) {
            return new plane(n);
        }
    }
    return nullptr;
...
{
    if (sscanf(str, "light((%lf, %lf, %lf), (%lf, %lf, %lf))", &l.pos.x,
               &l.pos.y, &l.pos.z, &l.col.r, &l.col.g, &l.col.b) == 6) {
        auto f = [](double s) { return 0 <= s && s <= 1; };
        if (f(l.col.r) && f(l.col.g) && f(l.col.b))
            return true;
    }
    return false;
}
...
bool parse_inline(int argc, const char *const argv[], image_task &cfg)
{
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(argv[i], "-b") == 0) {
            if (++i >= argc)
                return false;
            if (not parse_division(argv[i], cfg.dd))
                return false;
            continue;
        }
        if (strcmp(argv[i], "-c") == 0) {
            if (++i >= argc)
                return false;
            if (not parse_camera(argv[i], cfg.cam))
                return false;
            continue;
        }
        ...
        if (strcmp(argv[i], "-r") == 0) {
            if (++i >= argc)
                return false;
            if (sscanf(argv[i], "%hu,%hu", &cfg.j, &cfg.i) != 2)
                return false;
            cfg.single = true;
            continue;
        }
